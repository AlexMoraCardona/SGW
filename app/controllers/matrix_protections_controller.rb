class MatrixProtectionsController < ApplicationController
    def index
        ...
        if  Current.user && Current.user.level > 0 && Current.user.level < 3
            if params[:entity_id].present?
                ...
                @matrix_protections = MatrixProtection.where("entity_id = ?", params[:entity_id]).order(id: :desc)
            else 
                @entities = Entity.all
            end    
        elsif Current.user && Current.user.level > 2 
            @entity = Entity.find(Current.user.entity)
            @matrix_protections = MatrixProtection.where("entity_id = ?",Current.user.entity).order(id: :desc)
        else
    ...
    def firmar_rep 
        ...
        if params[:format].to_i == 1
            if  @matrix_protection.user_legal_representative.to_i == Current.user.id.to_i
    ...
    def firmar_adv
        ...
        if params[:format].to_i == 2
            if  @matrix_protection.user_adviser_sst.to_i == Current.user.id.to_i
    ...
    def firmar_res
        ...
        if params[:format].to_i == 3
            if  @matrix_protection.user_responsible_sst.to_i == Current.user.id.to_i
